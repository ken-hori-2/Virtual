...
class Agent():
    ...
    def mdp(self, state, TRIGAR, action,     states_known, map, grid, DIR):
        ...
        # from env_MP import Env_MP
        # env_mp = Env_MP(grid, state)
        ...
        if self.NODELIST[state.row][state.column] in pre:
            ...
            a, v, aaa = test.plan(states_known, map, state, DIR)

            "仮想座標"
            # a, v, aaa = test.plan(states_known2, m, state_init, DIR) 
            # print(aaa)
            # import pprint
            # pprint.pprint(a)
            # pprint.pprint(v)
