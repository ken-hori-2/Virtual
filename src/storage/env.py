...
class Environment():
    ...
    def transit_func_MP(self, state, action):
        transition_probs = {}
    ...
    def Distance(self, s):
        ...
        x = s.row
        y = s.column
        self.goal = (0, 4) # small-grid
        self.goal = (0, 18) # large-grid
        # self.goal = (18, 0) # ゴール方向を間違えた情報にする場合
        ...
        self.goal = (self.agent_state.row+(-UP+DOWN), self.agent_state.column+(-LEFT+RIGHT)) # Virtual Goal
        # print(f"----- {self.goal} -----")
        "----- Add 0206 -----"

        # self.start = (4, 0)
        self.start = (self.agent_state.row, self.agent_state.column)
        # print("start pose :", self.start)
    ...
    def step_MP(self, state, action, map, stress):
        self.agent_state = state
        next_state, reward, done = self.transit_MP(self.agent_state, action, map, stress)
        if next_state is not None:
            self.agent_state = next_state

        "Direction"
        attribute_agent = self.grid[self.agent_state.row][self.agent_state.column]
