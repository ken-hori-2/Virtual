...
class Planner():
    ...
    def dict_to_grid(self, state_reward_dict):
        grid = []
        for i in range(3): # self.env.row_length):
            row = [0] * 3 # self.env.column_length
            grid.append(row)
...
class ValueIterationPlanner(Planner):
    ...
    def plan(self, state_known, map, init, DIR, action_list, gamma=0.9, threshold=0.0001):
        ...
        while True:
            ...
            for s in self.state_known:
                ...
                if not self.env.can_action_at_MP(s):
                    continue
                expected_rewards = []
    ...
    def draw_arrow(self, grid, V):
        ...
        for s in V:
            ...
            if self.grid_virtual[s.row][s.column] == 1:
    ...
    def show(self, A, V, state, map,     DIR):
        ...
        try:
            max_dir = max(DIR)
            if state.row+(-DIR[0]+DIR[1])/max_dir < 0 or state.column+(-DIR[2]+DIR[3])/max_dir < 0 or state.row+(-DIR[0]+DIR[1])/max_dir >= -size or state.column+(-DIR[2]+DIR[3])/max_dir >= -size:
                pass
            else:
                ...
                plt.plot(state.column+(-DIR[2]+DIR[3])/max_dir, -state.row+(DIR[0]-DIR[1])/max_dir, marker = "*", color = "orange", markersize=10,     label = "Estimated") # , alpha = 0.5)
                plt.plot([state.column, state.column+(-DIR[2]+DIR[3])/max_dir], [-state.row, -state.row+(DIR[0]-DIR[1])/max_dir], linestyle = "--", color='orange', alpha = 0.5)
        except:
            pass
